...
ifd_table.columns = ifd_table.columns.str.lower().str.replace(' ', '_')
    ...
    for y in YEARS_ORDER:
        ...
        for metric in ['dmax', 'hmax', 'vmax', 'z0max']:
            ...
            if not row.empty:
                ...
                val = sample_raster_value(lat, lon, path, prefer_band=1)
...
#             val = list(src.sample([(xs[0], ys[0])]))[0][0]
#             if val in (src.nodata, 255, -9999) or val is None or val < 0:
#                 return np.nan
#             if val > 20: val /= 100.0  # cm→m conversion heuristic
#             return round(float(val), 3)
#     except Exception as e:
#         print(f"⚠️ Raster read error at {raster_path}: {e}")
#         return np.nan
# def sample_raster_value(lat, lon, raster_path, prefer_band=1):
#     """
...
# def sample_raster_value(lat, lon, raster_path, prefer_band=1):
#     """Sample raster at given lat/lon. Returns np.nan if outside extent."""
#     try:
#         with rasterio.open(raster_path) as src:
#             # Print for debugging
#             # print(f"Sampling {raster_path}, CRS={src.crs}, bounds={src.bounds}")
...
#                 return np.nan
#             if val < 0:
#                 return 0.0  # treat dry cells as 0
#             if val > 20:
#                 val /= 100.0  # cm→m correction
#             return round(float(val), 3)
#     except Exception as e:
#         print(f"⚠️ Raster error for {raster_path}: {e}")
#         return np.nan


# def sample_raster_value(lat, lon, raster_path, prefer_band=1):
#     """Sample raster safely at given lat/lon. Handles nodata, scaling, and units."""
#     import rasterio
#     from rasterio.warp import transform
#     import numpy as np
...
#     try:
#         with rasterio.open(raster_path) as src:
#             # Convert WGS84 → local CRS (usually EPSG:28355)
#             xs, ys = transform("EPSG:4326", src.crs, [lon], [lat])

#             # Check if point is inside raster
#             if not (src.bounds.left <= xs[0] <= src.bounds.right and src.bounds.bottom <= ys[0] <= src.bounds.top):
#                 return np.nan

#             val = list(src.sample([(xs[0], ys[0])], indexes=prefer_band))[0][0]

#             # Get nodata value from raster metadata (if present)
#             nodata_val = src.nodata if src.nodata is not None else 255

#             # Handle invalid or missing data
#             if val in (None, nodata_val, -9999) or np.isnan(val):
#                 return 0.0

#             # Scaling logic
#             if src.dtypes[0] == "uint8":
#                 # Common in flood hazard rasters: 0–255 range
#                 if val >= 250:
#                     return 0.0  # treat 255 as no flood
#                 val = val / 100.0  # cm → m
#             elif val > 100:
#                 val = val / 1000.0  # if integer meters
...
#     except Exception as e:
#         print(f"⚠️ Raster sampling error ({raster_path}): {e}")
#         print(f"{raster_path}: raw value={val}, nodata={src.nodata}")
